...
const DocumentDetailPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <Link
            ...
            className="px-3.5 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white/70 dark:bg-slate-900/70 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 transition-colors"
          >
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 grid grid-cols-1 md:grid-cols-4 gap-6 items-center shadow-lg">
        {/* Risk Gauge */}
        <div className="md:col-span-1 flex flex-col items-center justify-center p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200/80 dark:border-slate-800/80 text-center">
          ...
          <div className="flex items-baseline gap-1 my-1">
            <span
              className={`text-4xl font-extrabold font-mono ${
                score >= 60 ? 'text-rose-500' : score >= 35 ? 'text-amber-500' : 'text-emerald-500'
              }`}
            >
              {score}
            </span>
        ...
        <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div>
            ...
            <div className="space-y-1">
              {(analysis.parties || []).map((party, i) => (
                <div key={i} className="text-xs font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
          ...
          <div>
            ...
            <div className="text-xs text-slate-700 dark:text-slate-300">
              ...
              <span className="text-slate-400 ml-1">High-Risk clauses requiring immediate counter-proposals</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 overflow-x-auto space-x-2">
        {[
          ...
          { id: 'risks', label: `Risk Sentinel Engine (${risks.length})`, icon: ShieldAlert, badge: risks.length },
      ...
      {activeTab === 'summary' && (
        <div className="space-y-6 animate-fade-in">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="glass-card rounded-3xl p-6 sm:p-8 border border-brand-500/30 bg-brand-500/[0.02]">
              <div className="flex items-center gap-2 text-brand-600 dark:text-brand-400 mb-4">
                <Sparkles className="w-5 h-5" />
                <h3 className="text-base font-bold">Plain-English Translation</h3>
              </div>
            ...
            <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2 text-slate-800 dark:text-white mb-4">
                ...
                <h3 className="text-base font-bold">Executive Legal Summary</h3>
              </div>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="space-y-4">
            {risks.map((risk, index) => (
              <div
                key={risk.id || index}
                className="glass-card rounded-3xl p-6 sm:p-7 border border-slate-200 dark:border-slate-800 hover:border-brand-500/40 transition-all shadow-sm"
              >
                <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                  ...
                  {risk.clauseRef && (
                    <span className="px-2.5 py-1 rounded-md text-[11px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                      {risk.clauseRef}
                    </span>
                  )}
                </div>
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 rounded-xl bg-brand-500/5 dark:bg-brand-500/10 border border-brand-500/20">
                    <span className="text-[10px] uppercase font-bold text-brand-600 dark:text-brand-400 block mb-1 flex items-center gap-1">
      ...
      {activeTab === 'obligations' && (
        <div className="glass-card rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm animate-fade-in">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">Extracted Contractual Obligations</h3>
                ...
                {obligations.map((ob, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2.5 py-1 rounded-md text-[10px] font-semibold bg-brand-500/10 text-brand-600 dark:text-brand-400">
                        {ob.type}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-300 leading-relaxed">
                      {ob.obligation}
                    </td>
                  </tr>
                ))}
      ...
      {activeTab === 'timeline' && (
        <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 animate-fade-in">
          <h3 className="text-base font-bold text-slate-900 dark:text-white mb-6">Contract Milestone & Notice Timeline</h3>
          <div className="relative border-l-2 border-brand-500/30 ml-4 space-y-8 pb-4">
            {deadlines.map((dl, index) => (
              <div key={index} className="relative pl-7 group">
                {/* Dot */}
                <div
                  className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-white dark:border-navy-950 ${
                    dl.urgency === 'High'
      ...
      {activeTab === 'raw' && (
        <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 animate-fade-in space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Raw Extracted Contract Text</h3>
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 rounded-xl text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-500"
              />
            </div>
          </div>

          <div className="p-4 rounded-2xl bg-slate-900 text-slate-300 font-mono text-xs overflow-x-auto max-h-[600px] overflow-y-auto leading-relaxed whitespace-pre-wrap select-text">
            {rawSearch ? (
              document.extractedText
                ?.split(new RegExp(`(${rawSearch})`, 'gi'))
                .map((part, i) =>
                  part.toLowerCase() === rawSearch.toLowerCase() ? (
                    <mark key={i} className="bg-amber-400 text-slate-950 font-bold rounded px-0.5">
                      {part}
                    </mark>
                  ) : (
                    part
